...
* Performance:
  ...
  * Optimal integer data-type implementation: based on the `maxIntegerValue` configuration option (expected maximum absolute value in XACML attributes of type `http://www.w3.org/2001/XMLSchema#integer`), the PDP chooses the most efficient Java data-type. Typically, the default and larger Java type for `http://www.w3.org/2001/XMLSchema#integer` is `BigInteger`. Obviously, this is overkill for an integer attribute representing the age of the subject; in this case the `Short` type is more than enough, and especially more efficient. Overall, decreasing the `maxIntegerValue` value as much as possible, based on the range you expect your integer values to be in, makes the PDP engine more efficient on integer handling: lower memory consumption, faster computations.
