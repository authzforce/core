 ...
 * 
 */
public final class PolicySetEvaluator extends oasis.names.tc.xacml._3_0.core.schema.wd_17.PolicySet implements IPolicy
{
	...
	private final GenericPolicyEvaluator<IPolicy> genericPolicyEvaluator;
	 ...
	 */
	public PolicySetEvaluator(oasis.names.tc.xacml._3_0.core.schema.wd_17.PolicySet policySetElement, XPathCompiler parentDefaultXPathCompiler, Expression.Factory expressionFactory, CombiningAlgRegistry combiningAlgorithmRegistry, RefPolicyFinder refPolicyFinder, Deque<String> policySetRefChain)
			throws ParsingException
	{
		...
		try
		{
			evaluatableTarget = new TargetEvaluator(policySetElement.getTarget(), defaultXPathCompiler, expressionFactory);
		...
		for (final Object policySetChildElt : policySetElement.getPolicySetsAndPoliciesAndPolicySetIdReferences())
		{
			if (policySetChildElt instanceof PolicyCombinerParameters)
			{
				...
				final CombinerElement<PolicyEvaluator> combinerElt;
				try
				{
			...
			} else if (policySetChildElt instanceof PolicySetCombinerParameters)
			{
				...
				final CombinerElement<PolicySetEvaluator> combinerElt;
				try
				{
			...
			} else if (policySetChildElt instanceof JAXBElement)
			{
				...
				if (eltNameLocalPart.equals(XACMLNodeName.POLICY_ID_REFERENCE.value()))
				{
					...
					combinedChildElements.add(PolicyReference.getInstance(idRef, refPolicyFinder, PolicyEvaluator.class, null));
				} else if (eltNameLocalPart.equals(XACMLNodeName.POLICYSET_ID_REFERENCE.value()))
				{
					...
					final IdReferenceType idRef = (IdReferenceType) jaxbElt.getValue();
					combinedChildElements.add(PolicyReference.getInstance(idRef, refPolicyFinder, PolicySetEvaluator.class, policySetRefChain));
			...
			} else if (policySetChildElt instanceof oasis.names.tc.xacml._3_0.core.schema.wd_17.PolicySet)
			{
				...
				try
				{
					childPolicySet = new PolicySetEvaluator((oasis.names.tc.xacml._3_0.core.schema.wd_17.PolicySet) policySetChildElt, defaultXPathCompiler, expressionFactory, combiningAlgorithmRegistry, refPolicyFinder, policySetRefChain);
				} catch (ParsingException e)
				{
			...
			} else if (policySetChildElt instanceof oasis.names.tc.xacml._3_0.core.schema.wd_17.Policy)
			{
				...
				try
				{
					childPolicy = new PolicyEvaluator((oasis.names.tc.xacml._3_0.core.schema.wd_17.Policy) policySetChildElt, defaultXPathCompiler, expressionFactory, combiningAlgorithmRegistry);
				} catch (ParsingException e)
				{
		...
		this.genericPolicyEvaluator = new GenericPolicyEvaluator<>(evaluatableTarget, combinedChildElements, policyCombinerParameters, policyCombiningAlg, pepActionExps, Collections.EMPTY_SET, toString);
	}
	...
	public boolean equals(Object obj)
	{
		...
		if (!(obj instanceof PolicySetEvaluator))
		{
			return false;
		}

		final PolicySetEvaluator other = (PolicySetEvaluator) obj;
		/*
