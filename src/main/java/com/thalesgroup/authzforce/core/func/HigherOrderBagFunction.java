...
public abstract class HigherOrderBagFunction<T extends ExpressionResult<? extends AttributeValue>, SUB_RETURN_PRIMITIVE_T extends AttributeValue> extends Function<T>
{
	...
	@Override
	public final FunctionCall<T> parseInputs(List<Expression<? extends ExpressionResult<? extends AttributeValue>>> inputs) throws IllegalArgumentException
	{
		...
		/*
		 * Check whether it is a BaseFunction because it is the only type of function for which we
		 * have a generic way to validate argument types as done later below
		 */
		if (!(inputFunc instanceof BaseFunction))
		{
		...
		// so now we know we have a boolean BaseFunction
		@SuppressWarnings("unchecked")
		final BaseFunction<PrimitiveResult<? extends SUB_RETURN_PRIMITIVE_T>> subFunc = (BaseFunction<PrimitiveResult<? extends SUB_RETURN_PRIMITIVE_T>>) inputFunc;
	 ...
	 */
	protected abstract FunctionCall<T> createFunctionCallFromSubFunction(BaseFunction<PrimitiveResult<? extends SUB_RETURN_PRIMITIVE_T>> subFunc, List<Expression<? extends ExpressionResult<? extends AttributeValue>>> inputsAfterSubFunc);
	...
	static abstract class OneBagOnlyFunction<RETURN_T extends ExpressionResult<? extends AttributeValue>, SUB_RETURN_T extends AttributeValue> extends HigherOrderBagFunction<RETURN_T, SUB_RETURN_T>
	{
		 ...
		 */
		protected abstract RETURN_T evaluate(BaseFunction<PrimitiveResult<? extends SUB_RETURN_T>>.Call subFuncCall,
		/* Collection<? extends AttributeValue> */AttributeValue[] lastArgBag, int lastArgIndex, EvaluationContext context) throws IndeterminateEvaluationException;

		private final String errorEvalLastArgMsg = "Function " + this + ": Error evaluating last arg (bag)";

		public final RETURN_T evaluate(BaseFunction<PrimitiveResult<? extends SUB_RETURN_T>>.Call subFuncCall, Expression<? extends ExpressionResult<? extends AttributeValue>> lastArgBagExp, int lastArgIndex, EvaluationContext context) throws IndeterminateEvaluationException
		{
		...
		private class OneBagOnlyFunctionCall implements FunctionCall<RETURN_T>
		{
			...
			private final BaseFunction<PrimitiveResult<? extends SUB_RETURN_T>>.Call subFuncCall;
			private final Expression<? extends ExpressionResult<? extends AttributeValue>> lastArgBag;
			private final int lastArgIndex;

			protected OneBagOnlyFunctionCall(BaseFunction<PrimitiveResult<? extends SUB_RETURN_T>> subFunction, List<Expression<? extends ExpressionResult<? extends AttributeValue>>> primitiveInputs, Expression<? extends ExpressionResult<? extends AttributeValue>> lastInputBag)
			{
				this.subFuncCall = subFunction.parseInputs(primitiveInputs, new DatatypeDef(lastInputBag.getReturnType().datatypeURI()));
		 ...
		 * createFunctionCallFromSubFunction(com.thalesgroup.authzforce.core.func.BaseFunction,
		 * java.util.List, java.util.List)
		 */
		@Override
		protected final FunctionCall<RETURN_T> createFunctionCallFromSubFunction(BaseFunction<PrimitiveResult<? extends SUB_RETURN_T>> subFunc, List<Expression<? extends ExpressionResult<? extends AttributeValue>>> inputsAfterSubFunc)
		{
	...
	private static abstract class BooleanOneBagOnlyFunction extends OneBagOnlyFunction<PrimitiveResult<BooleanAttributeValue>, BooleanAttributeValue>
	{
		protected BooleanOneBagOnlyFunction(String functionId)
		{
			super(functionId, BooleanAttributeValue.TYPE, BooleanAttributeValue.identifier);
		}
	}
	...
	private static class AnyOf extends BooleanOneBagOnlyFunction
	{
		...
		@Override
		protected final PrimitiveResult<BooleanAttributeValue> evaluate(BaseFunction<PrimitiveResult<? extends BooleanAttributeValue>>.Call subFuncCall,
		/* Collection<? extends AttributeValue> */AttributeValue[] lastArgBagVals, int lastArgIndex, EvaluationContext context) throws IndeterminateEvaluationException
		{
			...
			{
				final PrimitiveResult<? extends BooleanAttributeValue> subResult;
				try
				{
				...
				final BooleanAttributeValue subResultVal = subResult.value();
				if (subResultVal.getValue())
				{
	...
	private static abstract class BooleanHigherOrderBagFunction extends HigherOrderBagFunction<PrimitiveResult<? extends BooleanAttributeValue>, BooleanAttributeValue>
	{
	...
	private static class AnyOfAny extends BooleanHigherOrderBagFunction
	{
		...
		@Override
		protected final FunctionCall<PrimitiveResult<? extends BooleanAttributeValue>> createFunctionCallFromSubFunction(BaseFunction<PrimitiveResult<? extends BooleanAttributeValue>> subFunc, List<Expression<? extends ExpressionResult<? extends AttributeValue>>> inputsAfterSubFunc)
		{
			return new AnyOfAnyFunctionCall(subFunc, inputsAfterSubFunc);
		}

		private class AnyOfAnyFunctionCall implements FunctionCall<PrimitiveResult<? extends BooleanAttributeValue>>
		{
			private final BaseFunction<PrimitiveResult<? extends BooleanAttributeValue>>.Call subFuncCall;
			...
			protected AnyOfAnyFunctionCall(BaseFunction<PrimitiveResult<? extends BooleanAttributeValue>> subFunc, List<Expression<? extends ExpressionResult<? extends AttributeValue>>> inputsAfterSubFunc)
			{
				...
				this.subFuncCall = subFunc.parseInputs(Collections.EMPTY_LIST, subFuncArgTypes);
				this.inputsAfterSubFunc = inputsAfterSubFunc;
			}

			@Override
			public final PrimitiveResult<? extends BooleanAttributeValue> evaluate(EvaluationContext context) throws IndeterminateEvaluationException
			{
				return AnyOfAny.this.evaluate(subFuncCall, inputsAfterSubFunc, subFuncArity, context);
			}
		...
		private final PrimitiveResult<? extends BooleanAttributeValue> eval(Iterator<Expression<? extends ExpressionResult<? extends AttributeValue>>> inputsAfterSubFuncIterator, ListIterator<AttributeValue[]> nonFirstArgEvalResultsIterator,
				BaseFunction<PrimitiveResult<? extends BooleanAttributeValue>>.Call subFuncCall, int subFuncArity, Deque<AttributeValue> subFuncArgsStack, EvaluationContext context) throws IndeterminateEvaluationException
		{
			...
			if (inputsAfterSubFuncIterator.hasNext())
			{
				...
				try
				{
					 ...
					 * AttributeDesignator/AttributeSelector with MustBePresent=False evaluates to
					 * empty bag.
					 */
					if (argVals.length == 0)
					{
			...
			/*
			 * No more arg expression to evaluate, but we may have evaluated them all with results
			 * put in nonFirstArgEvalResultsIterator, then started a new combination of arguments
			 * from the start, working with nonFirstArgEvalResultsIterator only after that So check
			 * where we are with nonFirstArgEvalResultsIterator
			 */
			...
			for (final AttributeValue argVal : argVals)
			{
				...
				final PrimitiveResult<? extends BooleanAttributeValue> subResult = eval(inputsAfterSubFuncIterator, nonFirstArgEvalResultsIterator, subFuncCall, subFuncArity, subFuncArgsStack, context);
				/*
				 * On each call to eval, nonFirstArgEvalResultsIterator.next()/add() is called, so
				 * we need to go backwards, when going back to the caller
				 */
				...
				/*
				 * Remove the arg we just added at the start of the iteration, to leave the place
				 * for the new arg in the next iteration
				 */
		...
		private final PrimitiveResult<? extends BooleanAttributeValue> evaluate(BaseFunction<PrimitiveResult<? extends BooleanAttributeValue>>.Call subFuncCall, List<Expression<? extends ExpressionResult<? extends AttributeValue>>> inputsAfterSubFunc, int subFuncArity, EvaluationContext context)
				throws IndeterminateEvaluationException
		{
			/*
			 * For each input expression coming from inputsAfterSubFunc, the evaluation result will
			 * be added to the following list, to avoid evaluating the same expression again as each
			 * one will be reused in multiple combination of arguments:
			 */
			 ...
			 * sub-function arguments because we will be pushing/popping for each value in each bag
			 * argument, to make the list of the sub-function's arguments before we can make each
			 * sub-function call.
			 */
			...
			return eval(inputsAfterSubFunc.iterator(), inputsAfterSubFuncEvalResults.listIterator(), subFuncCall, subFuncArity, subFuncArgsStack, context);
		}
	}
	...
	private static abstract class TwoBagFunction extends BooleanHigherOrderBagFunction
	{
		...
		private class TwoBagFunctionCall implements FunctionCall<PrimitiveResult<? extends BooleanAttributeValue>>
		{
			private final BaseFunction<PrimitiveResult<? extends BooleanAttributeValue>>.Call subFuncCall;
			...
			protected TwoBagFunctionCall(BaseFunction<PrimitiveResult<? extends BooleanAttributeValue>> subFunc, Expression<BagResult<? extends AttributeValue>> input0, Expression<BagResult<? extends AttributeValue>> input1)
			{
			...
			@Override
			public final PrimitiveResult<BooleanAttributeValue> evaluate(EvaluationContext context) throws IndeterminateEvaluationException
			{
		...
		protected final String SUB_FUNC_CALL_ERROR_MSG_PREFIX = "Function " + functionId + ": Error evaluating sub-function with arguments (evaluated to): ";

		protected abstract PrimitiveResult<BooleanAttributeValue> evaluate(BaseFunction<PrimitiveResult<? extends BooleanAttributeValue>>.Call subFuncCall, BagResult<? extends AttributeValue> bag0Vals, BagResult<? extends AttributeValue> bag1Vals, EvaluationContext context)
				throws IndeterminateEvaluationException;

		protected final PrimitiveResult<BooleanAttributeValue> evaluate(BaseFunction<PrimitiveResult<? extends BooleanAttributeValue>>.Call subFuncCall, Expression<BagResult<? extends AttributeValue>> inputBag0, Expression<BagResult<? extends AttributeValue>> inputBag1, EvaluationContext context)
				throws IndeterminateEvaluationException
		{
	...
	private static class AllOfAny extends TwoBagFunction
	{
		 ...
		 * .authzforce.core.func.BaseFunction, java.util.List, com.thalesgroup.authzforce.core.test.EvaluationCtx)
		 */
		@Override
		protected final PrimitiveResult<BooleanAttributeValue> evaluate(BaseFunction<PrimitiveResult<? extends BooleanAttributeValue>>.Call subFuncCall, BagResult<? extends AttributeValue> bag0, BagResult<? extends AttributeValue> bag1, EvaluationContext context)
				throws IndeterminateEvaluationException

		{
			...
			{
				// FIXME: reuse datatype from somewhere to avoid instantiating a new one
				subFuncArgStack.add(new PrimitiveResult<>(bag0Val, bag0.getReturnType()));
				boolean isAnyTrue = false;
				for (final AttributeValue bag1Val : bag1.values())
				{
					subFuncArgStack.add(new PrimitiveResult<>(bag1Val, bag1.getReturnType()));
					...
					{
						throw new IndeterminateEvaluationException(SUB_FUNC_CALL_ERROR_MSG_PREFIX + subFuncArgStack, Status.STATUS_PROCESSING_ERROR);
					}

					subFuncArgStack.removeLast();
					if (subResult.value().getValue())
					{
						isAnyTrue = true;
						break;
					}
