 ...
 * <p>
 * The sub-modules may very likely hold resources such as network resources to get attributes remotely, or attribute
 * caches to speed up finding, etc. Therefore, you are required to call {@link #close()} when you no longer need an
 * instance - especially before replacing with a new instance (with different modules) - in order to make sure these
...
public final class CloseableAttributeProvider extends ModularAttributeProvider implements Closeable
{
	...
	private CloseableAttributeProvider(final Map<AttributeGUID, AttributeProviderModule> modulesByAttributeId,
			final Set<ModuleAdapter> moduleClosers, final boolean strictAttributeIssuerMatch)
	{
	...
	private static final CloseableAttributeProvider EVALUATION_CONTEXT_ONLY_SCOPED_CLOSEABLE_ATTRIBUTE_PROVIDER = new CloseableAttributeProvider(
			Collections.<AttributeGUID, AttributeProviderModule>emptyMap(), Collections.<ModuleAdapter>emptySet(),
			true);

	/**
	 * Instantiates attribute Provider that tries to find attribute values in evaluation context, then, if not there,
	 * query the {@code module} providing the requested attribute ID, if any.
	 *
	 * @param attributeFactory
	 *            (mandatory) attribute value factory
	 * @param jaxbAttributeProviderConfs
	 *            (optional) XML/JAXB configurations of Attribute Providers for AttributeDesignator/AttributeSelector
	 ...
	 */
	public static CloseableAttributeProvider getInstance(
			final List<AbstractAttributeProvider> jaxbAttributeProviderConfs,
			final DatatypeFactoryRegistry attributeFactory, final boolean strictAttributeIssuerMatch,
			final EnvironmentProperties environmentProperties) throws IOException
	{
		...
		final Map<AttributeGUID, AttributeProviderModule> modulesByAttributeId = HashObjObjMaps.newUpdatableMap();
		final int moduleCount = jaxbAttributeProviderConfs.size();
		final Set<ModuleAdapter> mutableModuleCloserSet = HashObjSets.newUpdatableSet(moduleCount);
		for (final AbstractAttributeProvider jaxbAttributeProviderConf : jaxbAttributeProviderConfs)
		{
			try
			{
				...
				/*
				 * Each AttributeProviderModule is given a read-only AttributeProvider - aka
				 * "dependency attribute Provider" - to find any attribute they require (dependency), based on the
				 * attribute Provider modules that provide these required attributes (set above); read-only so that
				 * modules use this attribute Provider only to get required attributes, nothing else. Create this
				...
				else
				{
					...
					depAttrProvider = new ModularAttributeProvider(immutableCopyOfAttrProviderModsByAttrId,
							requiredAttrs, strictAttributeIssuerMatch);
				}
				...
				for (final AttributeDesignatorType attrDesignator : moduleAdapter.getProvidedAttributes())
				{
					...
					if (modulesByAttributeId.containsKey(attrGUID))
					{
						moduleAdapter.close();
						throw new IllegalArgumentException(
								"Conflict: " + moduleAdapter + " providing the same AttributeDesignator (" + attrGUID
		...
		return new CloseableAttributeProvider(modulesByAttributeId, HashObjSets.newImmutableSet(mutableModuleCloserSet),
