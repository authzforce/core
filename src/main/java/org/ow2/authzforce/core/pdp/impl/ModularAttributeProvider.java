...
public class ModularAttributeProvider implements AttributeProvider
{
			...
			throws IndeterminateEvaluationException
	{
		...
		{
			/*
			 * This error does not necessarily matter, it depends on whether the attribute is required, i.e. MustBePresent=true for AttributeDesignator/Selector
			 * So we let AttributeDesignator/Select#evaluate() method log the errors if MustBePresent=true. Here debug level is enough
			 */
			...
			/*
			 * NOTE: It might happen - e.g. in conformance test IIB033 (Request's resource-id attribute datatype is different from datatype used in Policy) -
			 * that context.getAttributeDesignatorResult(attributeGUID, bagDatatype) threw IndeterminateEvaluationException although a value for 'attributeGUID'
			 * exists in context, because the existing datatype is different from requested 'bagDatatype'. In this case, the call below will return false (the
			 * value should not be overridden). We don't care about the result; what matters is that the value is set to an empty bag if there was no value.
			 */
			context.putAttributeDesignatorResultIfAbsent(attributeGUID, result);
			return result;
		}
