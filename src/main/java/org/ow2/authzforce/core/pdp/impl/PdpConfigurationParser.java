...
public final class PdpConfigurationParser
{
	 ...
	 *             Invalid PDP configuration at {@code confLocation}
	 */
	public static PDPImpl getPDP(final String confLocation, final String catalogLocation,
	 ...
	 *             Invalid PDP configuration at {@code confLocation}
	 */
	public static PDPImpl getPDP(final File confFile, final String catalogLocation, final String extensionXsdLocation)
	 ...
	 *             Invalid PDP configuration at {@code confLocation}
	 */
	public static PDPImpl getPDP(final String confLocation, final PdpModelHandler modelHandler)
			throws IOException, IllegalArgumentException
	{
		File confFile = null;
		try
	 ...
	 */
	public static PDPImpl getPDP(final File confFile, final PdpModelHandler modelHandler)
			throws IOException, IllegalArgumentException
	{
	 ...
	 */
	public static PDPImpl getPDP(final Pdp pdpJaxbConf, final EnvironmentProperties envProps)
			throws IllegalArgumentException, IOException
	{
		...
		final Set<DatatypeFactory<?>> datatypeExtensions = HashObjSets
				.newUpdatableSet(datatypeExtensionIdentifiers.size());
		for (final String datatypeId : datatypeExtensionIdentifiers)
		{
			final DatatypeFactory<?> datatypeFactory = PdpExtensionLoader.getExtension(DatatypeFactory.class,
					datatypeId);
		...
		if (pdpJaxbConf.isUseStandardDatatypes())
		{
			...
			{
				datatypeFactoryRegistry = new ImmutableDatatypeFactoryRegistry(
						HashObjSets.newImmutableSet(stdRegistry.getExtensions(), datatypeExtensions));
			}
		}
		else
		{
		...
		final Set<Function<?>> nonGenericFunctionExtensions = HashObjSets
		...
		if (pdpJaxbConf.isUseStandardFunctions())
		{
			...
			{
				functionRegistry = new ImmutableFunctionRegistry(
						HashObjSets.newImmutableSet(stdRegistry.getNonGenericFunctions(), nonGenericFunctionExtensions),
		...
		final Set<CombiningAlg<?>> algExtensions = HashObjSets.newUpdatableSet(algExtensionIdentifiers.size());
		...
		final DecisionResultFilter decisionResultFilter = resultFilterId == null ? null
				: PdpExtensionLoader.getExtension(DecisionResultFilter.class, resultFilterId);
		...
		try
		{
			maxVarRefDepth = bigMaxVarRefDepth == null ? -1
					: org.ow2.authzforce.core.pdp.api.value.IntegerValue.intValueExact(bigMaxVarRefDepth);
		}
		...
		try
		{
			maxPolicyRefDepth = bigMaxPolicyRefDepth == null ? -1
					: org.ow2.authzforce.core.pdp.api.value.IntegerValue.intValueExact(bigMaxPolicyRefDepth);
		}
		catch (final ArithmeticException e)
		{
		...
		return new PDPImpl(datatypeFactoryRegistry, functionRegistry, pdpJaxbConf.getAttributeProviders(),
				maxVarRefDepth, enableXPath, combiningAlgRegistry, pdpJaxbConf.getRootPolicyProvider(),
				pdpJaxbConf.getRefPolicyProvider(), maxPolicyRefDepth, pdpJaxbConf.getRequestFilter(),
				pdpJaxbConf.isStrictAttributeIssuerMatch(), pdpJaxbConf.getStandardEnvAttributeSource(),
				decisionResultFilter, jaxbDecisionCache, pdpJaxbConf.getBadRequestStatusDetailLevel().intValue(),
				envProps);
	}
	...
	{
		/*
		 * A function is said "XPath-based" iff it takes at least one XPathExpression parameter. Regarding higher-order
		 * function, as of now, we only provide higher-order functions defined in the XACML (3.0) Core specification,
		 * which are not XPath-based, or if a higher-order function happens to take a XPathExpression parameter, it is
		 * actually a parameter to the first-order sub-function. Plus it is not possible to add extensions that are
		 * higher-order functions in this PDP implementation. Therefore, it is enough to check first-order functions
		 * (class FirstOrderFunction) only. (Remember that such functions may be used as parameter to a higher-order
		 * function.)
		 */
