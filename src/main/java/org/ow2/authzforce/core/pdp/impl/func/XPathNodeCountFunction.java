 ...
 * <p>
 * From XACML core specification of function 'urn:oasis:names:tc:xacml:3.0:function:xpath-node-count': This function
 * SHALL take an 'urn:oasis:names:tc:xacml:3.0:data-type:xpathExpression' as an argument and evaluates to an
 * 'http://www.w3.org/2001/XMLSchema#integer'. The value returned from the function SHALL be the count of the nodes
 * within the node-set that match the given XPath expression. If the &lt;Content&gt; element of the category to which
...
final class XPathNodeCountFunction extends SingleParameterTypedFirstOrderFunction<IntegerValue, XPathValue>
{
	...
	private static final class CallFactory
	{
		private static final class Call extends FirstOrderFunctionCall<IntegerValue>
		{
			...
			private Call(FirstOrderFunctionSignature<IntegerValue> functionSig, List<Expression<?>> argExpressions,
					Datatype<?>[] remainingArgTypes) throws IllegalArgumentException
			{
				...
				invalidArgTypeMsg = "Function " + functionSig.getName() + ": Invalid type (expected = "
						+ StandardDatatypes.XPATH_FACTORY.getDatatype() + ") of arg#0: ";
				indeterminateArgMsg = "Function " + functionSig.getName() + ": Indeterminate arg #0";
				indeterminateArgEvalMsg = "Function " + functionSig.getName()
						+ ": Error evaluating xpathExpression arg #0";
			}

			@Override
			public IntegerValue evaluate(EvaluationContext context, AttributeValue... remainingArgs)
					throws IndeterminateEvaluationException
			{
				...
				if (checkedArgExpressions.isEmpty())
				{
					...
					{
						throw new IndeterminateEvaluationException(invalidArgTypeMsg + remainingArgs[0].getDataType(),
				...
				else
				{
					...
					{
						throw new IndeterminateEvaluationException(indeterminateArgMsg,
								StatusHelper.STATUS_PROCESSING_ERROR, e);
					}
				}
				...
				{
					throw new IndeterminateEvaluationException(indeterminateArgEvalMsg,
							StatusHelper.STATUS_PROCESSING_ERROR, e);
				}
		...
		protected FirstOrderFunctionCall<IntegerValue> getInstance(final List<Expression<?>> argExpressions,
	...
	XPathNodeCountFunction(String functionId)
	{
		super(functionId, StandardDatatypes.INTEGER_FACTORY.getDatatype(), true,
				Arrays.asList(StandardDatatypes.XPATH_FACTORY.getDatatype()));
