...
final class CombiningAlgEvaluators {
	...
	private static abstract class ConstantDecisionEvaluator implements CombiningAlg.Evaluator {
		...
		@Override
		public final ExtendedDecision evaluate(final EvaluationContext context,
	...
	/*
	 * Evaluator that uses the result of RulesWithSameEffectEvaluator to return
	 * the opposite if NotApplicable or Indeterminate{DP} if Indeterminate, else
	 * the same result.
	 */
	static class RulesWithSameEffectEvaluator implements CombiningAlg.Evaluator {
		...
		RulesWithSameEffectEvaluator(Collection<? extends RuleEvaluator> rulesWithSameEffect) {
			assert rulesWithSameEffect != null && !rulesWithSameEffect.isEmpty();
			// first rule's effect assumed the same for all
			this.commonDecision = rulesWithSameEffect.iterator().next().getEffect() == EffectType.DENY
