...
/**
 * 
 * Standard "unordered" *-overrides combining algorithm, as opposed to
 * ordered-*-overrides algorithms
 *
 */
abstract class DPOverridesCombiningAlg extends BaseCombiningAlg<Decidable> {
	/**
	 * "Ordered" Deny/Permit-overrides combining algorithm evaluator; "ordered"
	 * means combined elements are evaluated in the same order as in the input
	 * collection (constructor parameter 'combinedElements'), i.e. no
	 * re-ordering.
	 */
	private static final class OrderedDPOverridesCombiningAlgEvaluator extends BaseCombiningAlg.Evaluator<Decidable> {
		/**
		 * Helper to combine (not-overriding) decision results until a decision
		 * overrides or there is no more element to combine
		 */
		private static final class DecisionResultCollector {
			...
			/*
			 * Replaces atLeastOneErrorDP from XACML spec. atLeastOneErrorDP ==
			 * true <=> firstIndeterminateDPResult != null
			 */
			private ExtendedDecision firstIndeterminateDPResult = null;
			/*
			 * Replaces atLeastOneError${overriding_effect} from XACML spec.
			 * atLeastOneError${overriding_effect} == true <=>
			 * firstIndeterminate${overriding_effect} != null
			 */
			private ExtendedDecision firstIndeterminateOverridingEffect = null;
			/*
			 * Replaces atLeastOneError${overridden_effect} from XACML spec.
			 * atLeastOneError${overridden_effect} == true <=>
			 * firstIndeterminate${overridden_effect} != null
			 */
			private ExtendedDecision firstIndeterminateOverriddenEffect = null;

			/**
			 * Replaces atLeastOnePermit (resp. atLeastOneDeny) from description
			 * of permit-overrides (resp. deny-overrides) in the XACML spec.
			 * <p>
			 * atLeastOnePermit (resp. atLeastOneDeny) == false <=>
			 * combinedPepActions == null.
			 * <p>
			 * At this point, we don't know yet whether the PEP actions of
			 * combined/children's Permit/Deny decisions will be added to the
			 * final result's PEP actions, since we don't know yet whether the
			 * final decision is Permit/Deny.
			 */
			private UpdatablePepActions combinedPepActions = null;

			DecisionResultCollector(final boolean returnApplicablePolicyIdList) {
				/*
				 * Since we may combine multiple elements before returning a
				 * final decision, we have to collect them in a list; and since
				 * we don't know yet whether the final decision is
				 * NotApplicable, we cannot add collected applicable policies
				 * straight to outApplicablePolicyIdList. So we create a
				 * temporary list until we know the final decision applies.
				 */
			...
			/**
			 * Return new result's applicable policies combined (added last)
			 * with the ones previously found, or only the ones combined so far
			 * if result == null
			 * 
			 */
			 ...
			 * applicable policies and PEP actions), i.e. a Permit (resp. Deny)
			 * result for deny-overrides (resp. permit-overrides)
			 */
			...
			/**
			 * Add intermediate (not final a priori)
			 * Indeterminate${overriding_effect} result (update applicable
			 * policies, etc.)
			 */
			...
			/**
			 * Add intermediate (not final a priori)
			 * Indeterminate${overridden_effect} result (update applicable
			 * policies, etc.)
			 */
			...
			/**
			 * Add intermediate (not final a priori) IndeterminateDP result
			 * (update applicable policies, etc.)
			 */
		...
		private interface SubDecisionHandler {
			 ...
			 *            used to collect the result (added/combined with
			 *            previous ones), if the decision does not override, to
			 *            be reused later at the end of the evaluation
			 * @return true iff the result overrides all others (is final), e.g.
			 *         Deny for deny-overrides
			 */
		...
		private static final class IndeterminateSubDecisionHandler implements SubDecisionHandler {
			...
			private IndeterminateSubDecisionHandler(
					final Map<DecisionType, SubDecisionHandler> indeterminateResultHandlersByExtendedIndeterminateType) {
			...
			public boolean handle(final DecisionResult result, final DecisionResultCollector resultCollector) {
				...
				return this.indeterminateResultHandlersByExtendedIndeterminateType.get(extIndeterminate).handle(result,
						resultCollector);
			}
		...
		private final Map<DecisionType, SubDecisionHandler> resultHandlersByDecisionType = new EnumMap<>(
				DecisionType.class);

		OrderedDPOverridesCombiningAlgEvaluator(final Iterable<? extends Decidable> combinedElements,
			...
			final Map<DecisionType, SubDecisionHandler> indeterminateResultHandlersByExtendedIndeterminateType = new EnumMap<>(
					DecisionType.class);
			indeterminateResultHandlersByExtendedIndeterminateType.put(DecisionType.INDETERMINATE,
					INDETERMINATE_DP_SUBDECISIONHANDLER);
			if (overridingEffect == EffectType.DENY) {
				...
				indeterminateResultHandlersByExtendedIndeterminateType.put(DecisionType.DENY,
						INDETERMINATE_OVERRIDING_EFFECT_SUBDECISIONHANDLER);
				resultHandlersByDecisionType.put(DecisionType.PERMIT, OVERRIDDEN_DP_SUBDECISIONHANDLER);
				indeterminateResultHandlersByExtendedIndeterminateType.put(DecisionType.PERMIT,
						INDETERMINATE_OVERRIDDEN_EFFECT_SUBDECISIONHANDLER);
			} else {
			...
			resultHandlersByDecisionType.put(DecisionType.INDETERMINATE,
					new IndeterminateSubDecisionHandler(indeterminateResultHandlersByExtendedIndeterminateType));
		}
		...
		public final ExtendedDecision evaluate(final EvaluationContext context, final UpdatablePepActions outPepActions,
			...
			final DecisionResultCollector resultCollector = new DecisionResultCollector(
			...
			for (final Decidable combinedElement : getCombinedElements()) {
				...
				final boolean isResultOverriding = resultHandlersByDecisionType.get(result.getDecision()).handle(result,
						resultCollector);

				/*
				 * XACML §7.18: Obligations & Advice: do not return
				 * Obligations/Advice of the rule, policy, or policy set that
				 * does not match the decision resulting from evaluating the
				 * enclosing policy set. For example, if the final decision is
				 * Permit, we should add to outPepActions only the PEP actions
				 * from Permit decisions (permitPepActions)
				 */
				if (isResultOverriding) {
					/*
					 * result overrides all others, return it right away after
					 * updating output applicable policies and PEP actions
					 */
			...
			/*
			 * There was no overriding Deny/Permit decision, i.e. Deny (resp.
			 * Permit) in case of deny-overrides (resp. permit-overrides) alg,
			 * else: if any Indeterminate{DP}, then Indeterminate{DP}
			 */
			...
			final List<JAXBElement<IdReferenceType>> combinedApplicablePolicies = resultCollector
					.getApplicablePolicies(null);
			final PepActions combinedPepActionsOfNotOverridingDP = resultCollector.getPepActions();
			final ExtendedDecision firstIndeterminateWithOverridingEffect = resultCollector
					.getFirstIndeterminateWithOverridingEffect();
			final ExtendedDecision firstIndeterminateWithOverriddenEffect = resultCollector
					.getFirstIndeterminateWithOverriddenEffect();

			/*
			 * ${overriding_effect} = Deny and ${overridden_effect} = Permit
			 * (resp. Deny) in case of deny-overrides (resp. permit-overrides)
			 * algorithm.
			 * 
			 * If any Indeterminate{overriding_effect}, then: if ( any
			 * Indeterminate{overridden_effect} or any decision with
			 * ${overridden_effect} ) -> Indeterminate{DP}; else ->
			 * Indeterminate{overriding_effect} (this is a simplified equivalent
			 * of the algo in the spec)
			 */
			/*
			 * atLeastOne${overridden_effect} == true <=>
			 * ${overridden_effect}PepActions != null
			 */
			if (firstIndeterminateWithOverridingEffect != null) {
				...
				return ExtendedDecisions.newIndeterminate(
						firstIndeterminateWithOverriddenEffect != null || combinedPepActionsOfNotOverridingDP != null
								? DecisionType.INDETERMINATE : decisionForOverridingEffect.getDecision(),
						firstIndeterminateWithOverridingEffect.getStatus());
			}

			/*
			 * If we got a decision with overridden effect (e.g. Permit in case
			 * of deny-overrides algo) or Indeterminate{overridden_effect},
			 * return it, otherwise it's NOT_APPLICABLE
			 */
	...
	/**
	 * combining algorithm evaluator that evaluates rules with overriding Effect
	 * (in same order as in the constructor argument 'rulesWithOverridingEffect') before others
	 */
	private static final class OverridingEffectFirstRuleCombiningAlgEvaluator extends RulesWithSameEffectEvaluator {
		...
		/**
		 * Instantiates the evaluator a list of rules with same Effect,
		 * inferring the effect from the first rule in the list
		 * 
		 * @param rulesWithSameEffect
		 *            combined Rules, all expected to have the same Effect. Must
		...
		OverridingEffectFirstRuleCombiningAlgEvaluator(final Collection<RuleEvaluator> rulesWithOverridingEffect,
			...
			assert otherRules != null && !otherRules.isEmpty() && rulesWithOverridingEffect.iterator().next()
					.getEffect() != otherRules.iterator().next().getEffect();
		...
		/**
		 * Evaluate rules with overridden Effect in the case when the evaluation
		 * of the rules with overriding Effect returned Indeterminate
		 * 
		 * @param indeterminateFromRulesWithOverridingEffect
		 *            Indeterminate result from previous evaluation of rules
		 *            with overridING effect
		...
		private ExtendedDecision evaluateRulesWithOverriddenEffect(final EvaluationContext context,
			 ...
			 * Indeterminate{overriding_effect}, overriding_effect = D (resp. P)
			 * if overriding Effect is Deny (resp. Permit)
			 */
			...
			for (final RuleEvaluator rule : otherRules) {
				...
				if (evalResult.getDecision() != DecisionType.NOT_APPLICABLE) {
					 ...
					 * Indeterminate{overridden_effect}, which we have to
					 * combine with previous result (from rules with overriding
					 * Effect) Indeterminate{overriding_effect}. For example,
					 * <p>
			...
			/*
			 * All other rules (with overridden effect) NotApplicable -> initial
			 * Indeterminate result unchanged
			 */
		...
		/**
		 * Evaluate rules with overridden Effect in the case when the evaluation
		 * of the rules with overriding Effect returned NotApplicable
		 * 
		 * @return final decision
		 */
		private ExtendedDecision evaluateRulesWithOverriddenEffect(final EvaluationContext context,
			...
			for (final RuleEvaluator rule : otherRules) {
				...
				/*
				 * If the decision is Indeterminate, save the indeterminate
				 * cause for the final Indeterminate result (if first
				 * Indeterminate), only used if no other rule with determinate
				 * result checked above is found.
				 */
			...
			/*
			 * All decisions were NotApplicable or
			 * Indeterminate{overridden_effect}
			 */
		...
		public ExtendedDecision evaluate(final EvaluationContext context, final UpdatablePepActions updatablePepActions,
			...
			final ExtendedDecision extDecisionFromRulesWithOverridingEffect = super.evaluate(context,
					updatablePepActions, updatableApplicablePolicyIdList);
	...
	private interface RuleCollector {
		...
		void addRuleWithOverriddenEffect(RuleEvaluator rule);
		boolean hasRuleWithOverridingEffect();
		boolean hasRuleWithOverriddenEffect();
		Collection<? extends RuleEvaluator> getRulesWithOverridingEffect();
		Collection<? extends RuleEvaluator> getRulesWithOverriddenEffect();
	}
	
	private interface RuleCollectorFactory {
		RuleCollector getInstance();
	}
	
	/**
	 * 
	 * Rule collector that separates rules with overridding Effect from the others (e.g. Deny rules from Permit rules)
	 *
	 */
	private static class UnorderedDPOverridesRuleCollector implements RuleCollector {
		

		@Override
		public void addNonEmptyRuleWithOverridingEffect(RuleEvaluator rule) {
			// TODO Auto-generated method stub
			
		}

		@Override
		public void addRuleWithOverriddenEffect(RuleEvaluator rule) {
			// TODO Auto-generated method stub
		...
		@Override
		public Collection<? extends RuleEvaluator> getRulesWithOverridingEffect() {
			// TODO Auto-generated method stub
			return null;
		}

		@Override
		public Collection<? extends RuleEvaluator> getRulesWithOverriddenEffect() {
			// TODO Auto-generated method stub
			return null;
		}
	...
	public CombiningAlg.Evaluator getInstance(final Iterable<CombiningAlgParameter<? extends Decidable>> params,
		...
		if (!RuleEvaluator.class.isAssignableFrom(getCombinedElementType())) {
			return new OrderedDPOverridesCombiningAlgEvaluator(Preconditions.checkNotNull(combinedElements),
					EffectType.DENY);
		}
		...
		//////////////////// BEGIN DIFF, "unordered" *-overrides are different
		//////////////////// from ordered-*-overrides//////////////////
		/*
		 * We will reorder rules with overriding Effect (e.g. Deny for
		 * deny-overrides algorithm) before rules with overridden Effect (e.g.
		 * Permit for deny-overrides algorithm) since order does not matter and
		 * deny decision prevails
		 */
		...
		//////////////////// DIFF - final RuleCollector (interface) =
		//////////////////// ruleCollectorFactory.getInstance();
		// boolean atLeastOneNonEmptyRuleWithOverridingEffect = false
		// boolean atLeastOneRuleWithOverriddenEffect = false
		// final Deque<RuleEvaluator> reducedRules = new ArrayDeque<>();
		////////////// END DIFF

		/*
		 * 
		 * If we find any empty Rule in overridden Effect (no
		 * target/condition/pep_action), we don't need to look at other rules
		 * with such Effect; since such rule is enough to return the overridden
		 * Effect as decision, if there is no rule with overriding Effect (e.g.
		 * if algorithm is deny-overrides, then if there is no applicable Deny
		 * rule, if there is any empty Permit rule, the result is Permit)
		 */
		/*
		 * if atLeastOneEmptyPermitRule (resp. atLeastOneEmptyDenyRule) in case
		 * of deny-overrides (resp. permit-overrides) algorithm
		 */
		...
		while (combinedEltIterator.hasNext()) {
			...
			if (rule.getEffect() == overridingEffect) {
				/*
				 * If rule's effect is the overriding Effect and it has no
				 * target/condition/pep_actions, then rule will always return
				 * this Effect -> {overriding_effect}-overrides alg always
				 * evaluates to ${overriding_effect} (ignore/remove all other
				 * rules). ({overriding_effect} = Permit if algorithm is
				 * Permit-overrides, or Deny if algorithm is Deny-overrides in
				 * this statement.)
				 */
				if (rule.isEmptyEquivalent()) {
					LOGGER.warn(
							...
							this, rule, this.overridingEffect, this.overridingEffect, this.getId(),
							this.overridingEffect);
					return evaluatorIfEmptyRuleWithOverridingEffect;
				}

				/*
				 * Rule is not empty, i.e. has a target/condition/actions,
				 * therefore may not necessarily return its (overriding) Effect
				 * as decision
				 */

				/// BEGIN DIFF
				nonEmptyRulesWithOverridingEffect.add(rule);
				// DIFF - ruleCollector.addNonEmptyRuleWithOverridingEffect(rule)
				// atLeastOneNonEmptyRuleWithOverridingEffect = true
				// reducedRules.addLast(rule)
				// END DIFF
			...
			/*
			 * Rule Effect = {overridden_Effect} (e.g. Permit if algorithm is
			 * deny-overrides)
			 * 
			 * Process rule with overridden Effect only if no empty Rule with
			 * such Effect has been found yet. Indeed, as mentioned earlier, if
			 * there is an empty rule with such overridden Effect, we already
			 * know that the result is always the overridden Effect (so no need
			 * to process other rules with such Effect), if there is no
			 * applicable rule with overridING Effect. Indeed, only rules with
			 * overriding Effect may change the final result in this case.
			 */
			if (firstEmptyRuleWithOverriddenEffect != null) {
				...
				LOGGER.warn(
						...
						this, rule, overriddenEffect, firstEmptyRuleWithOverriddenEffect, overriddenEffect, overridingEffect);
				// continue looking for rules with overriding Effect
				continue;
			}

			// No empty Rule with overridden Effect found yet; what about this
			// one?
			if (rule.isEmptyEquivalent()) {
				/*
				 * This is the first declared empty Rule with overridden Effect
				 * -> always returns the overridden Effect as decision; we can
				 * ignore/remove other Rules with overridden Effect (have no
				 * effect anymore)
				 */
				LOGGER.warn(
						...
						this, rule, overriddenEffect, overriddenEffect, this.getId(), overriddenEffect,
						overridingEffect, overriddenEffect);
				firstEmptyRuleWithOverriddenEffect = rule;
				
				// BEGIN DIFF
				rulesWithOverriddenEffect.clear();
				rulesWithOverriddenEffect.addLast(rule);
				// DIFF - ruleCollector.addRuleWithOverriddenEffect(rule)
				// atLeastOneRuleWithOverriddenEffect = true
				// reducedRules.addLast(rule);
				// END DIFF
		...
		/*
		 * There is at least one rule and there is no empty Rule with overriding
		 * Effect.
		 * 
		...
		if (
				/// BEGIN DIFF
				nonEmptyRulesWithOverridingEffect.isEmpty()
		// DIFF - if(!ruleCollector.hasRuleWithOverridingEffect() )
		// if(!atLeastOneNonEmptyRuleWithOverridingEffect)
		// END DIFF
				)
		{
			/*
			 * no Rule with overriding Effect (whether empty or not) -> at least
			 * one Rule with overriding Effect and all rules are rules with
			 * overriding Effect
			 */
			if (firstEmptyRuleWithOverriddenEffect != null) {
				/*
				 * no Rule with overriding Effect but one empty rule with
				 * overridden Effect -> final result is the overridden Effect
				 * always
				 */
				LOGGER.warn(
						...
						this, this.overriddenEffect, firstEmptyRuleWithOverriddenEffect, this.getId(), this.overriddenEffect,
						this.overriddenEffect);
				return evaluatorIfEmptyRuleWithOverriddenEffectAndNoneWithOverridingOne;
			}

			/*
			 * At least one Rule with overridden Effect and all rules are
			 * non-empty rules with same (overridden) Effect
			 */
			return new CombiningAlgEvaluators.RulesWithSameEffectEvaluator(
					/// BEGIN DIFF
					rulesWithOverriddenEffect
			// DIFF - ruleCollector.getRulesWithOverriddenEffect()
			// reducedRules
					);
			// END DIFF
		}
		...
		if (rulesWithOverriddenEffect.isEmpty()) {
			/// DIFF - if(!ruleCollector.hasRuleWithOverriddenEffect() )
			// if(!atLeastOneRuleWithOverriddenEffect)
			// END DIFF
			
			/*
			 * No rule with overridden Effect -> only non-empty rules with same
			 * overriding Effect
			 */

			return new CombiningAlgEvaluators.RulesWithSameEffectEvaluator(
					/// BEGIN DIFF
					nonEmptyRulesWithOverridingEffect
			// DIFF - ruleCollector.getRulesWithOverridingEffect()
			// reducedRules
			);
			// END DIFF
		}

		// At least one Rule with overridden Effect and only non-empty rules
		// with overriding Effect
		/// BEGIN DIFF
		return new OverridingEffectFirstRuleCombiningAlgEvaluator(nonEmptyRulesWithOverridingEffect,
				rulesWithOverriddenEffect);
		// DIFF - ruleCollector.getRulesWithOverridingEffect()
		// return new OrderedDPOverridesCombiningAlgEvaluator(reducedRules, this.overridingEffect)
		// END DIFF
	}
}
