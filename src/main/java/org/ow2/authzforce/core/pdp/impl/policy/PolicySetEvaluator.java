...
public final class PolicySetEvaluator extends GenericPolicyEvaluator<IPolicyEvaluator>
{
	...
	private PolicySetEvaluator(String policyId, String version, Target target, String policyCombiningAlgId,
			...
			Set<String> localVariableIDs, XPathCompiler defaultXPathCompiler, ExpressionFactory expressionFactory,
			CombiningAlgRegistry combiningAlgRegistry, List<String> longestPolicyRefChain,
			Map<String, PolicyVersion> staticRefPolicies) throws IllegalArgumentException
	{
		super(IPolicyEvaluator.class, JaxbXACMLUtils.XACML_3_0_OBJECT_FACTORY
				.createPolicySetIdReference(new IdReferenceType(policyId, version, null, null)), target,
				policyCombiningAlgId, policyEvaluators, policyCombinerParameters, obligationExpressions,
				adviceExpressions, localVariableIDs, defaultXPathCompiler, expressionFactory, combiningAlgRegistry);
		this.longestPolicyRefChain = longestPolicyRefChain == null ? null : Collections
				.unmodifiableList(longestPolicyRefChain);
		this.staticRefPolicies = staticRefPolicies == null ? null : Collections.unmodifiableMap(staticRefPolicies);
	}
	 ...
	 */
	public static PolicySetEvaluator getInstance(PolicySet policyElement, XPathCompiler parentDefaultXPathCompiler,
			Map<String, String> namespacePrefixesByURI, ExpressionFactory expressionFactory,
			CombiningAlgRegistry combiningAlgorithmRegistry, RefPolicyProvider refPolicyProvider,
			Deque<String> policySetRefChain) throws IllegalArgumentException
	{
		...
		} else
		{
			...
			} catch (IllegalArgumentException e)
			{
				throw new IllegalArgumentException(policyFriendlyId
						+ ": Invalid PolicySetDefaults/XPathVersion or XML namespace prefix/URI undefined", e);
			}
		}
		 ...
		 * PolicySet. They should not be visible outside the scope of this. final Set<String> variableIds = new
		 * HashSet<>();
		 */
		...
		/*
		 * Whenever a new policy reference comes up that is statically resolved, update this map. If any such reference
		 * is not statically referenced, the map is set to null and no longer updated.
		 */
		...
		for (final Object policySetChildElt : policyElement.getPolicySetsAndPoliciesAndPolicySetIdReferences())
		{
			if (policySetChildElt instanceof PolicyCombinerParameters)
			{
				...
				try
				{
					combinerElt = new BaseCombiningAlgParameter<>(combinedPolicy,
							((CombinerParametersType) policySetChildElt).getCombinerParameters(), expressionFactory,
							defaultXPathCompiler);
				} catch (IllegalArgumentException e)
				{
					throw new IllegalArgumentException(policyFriendlyId + ": invalid child #" + childIndex
			...
			} else if (policySetChildElt instanceof JAXBElement)
			{
				...
				if (eltNameLocalPart.equals(XACMLNodeName.POLICY_ID_REFERENCE.value()))
				{
					...
					final PolicyReferenceEvaluator<PolicyEvaluator> policyRef = GenericPolicyEvaluator
							.getPolicyRefEvaluator(idRef, refPolicyProvider, PolicyEvaluator.class, null);
				...
				} else if (eltNameLocalPart.equals(XACMLNodeName.POLICYSET_ID_REFERENCE.value()))
				{
					...
					final PolicyReferenceEvaluator<PolicySetEvaluator> childElement = GenericPolicyEvaluator
							.getPolicyRefEvaluator(idRef, refPolicyProvider, PolicySetEvaluator.class,
									policySetRefChain);
					...
					if (childLongestPolicyRefChain != null
							&& (longestPolicyRefChain == null || childLongestPolicyRefChain.size() > longestPolicyRefChain
									.size()))
					{
					...
					{
						final PolicyVersion childVersion = childElement.getPolicyVersion();
						...
						} else
						{
							...
							{
								// Add each statically referenced policy from the current child PolicySetIdReference
								for (final Entry<String, PolicyVersion> childRefPolicy : childStaticRefPolicies
										.entrySet())
								{
									updateStaticRefPolicies(policyFriendlyId, staticallyReferencedPolicies,
											childRefPolicy.getKey(), childRefPolicy.getValue());
								}

								/*
								 * Add the current child PolicySetIdreference itself (childElement.getPolicyId(),
								 * childElement.getPolicyVersion())
								 */
								updateStaticRefPolicies(policyFriendlyId, staticallyReferencedPolicies,
										childElement.getPolicyId(), childElement.getPolicyVersion());
							}
						}
					}
			...
			} else if (policySetChildElt instanceof PolicySet)
			{
				...
				{
					/*
					 * This child PolicySet may have PoliSetIdReferences as well and therefore update the
					 * policySetRefChain and staticallyReferencedPolicies. However, if the current policySetRefChain is
					 * updated directly by a child PolicySet instantiation, then it is no longer valid for the other
					 * child PolicySets of this same PolicySet. So we need to pass a copy to
					 * PolicySetEvaluator.getInstance(() to avoid that inconsistency.
					 */
					childElement = PolicySetEvaluator.getInstance((PolicySet) policySetChildElt, defaultXPathCompiler,
				...
				// update staticallyReferencedPolicies (if not null)
				if (staticallyReferencedPolicies != null)
				{
					final Map<String, PolicyVersion> childStaticRefPolicies = childElement.getStaticRefPolicies();
					if (childStaticRefPolicies == null)
					{
						staticallyReferencedPolicies = null;
					} else
					{
						// Add each statically referenced policy in the current child PolicySet
						for (final Entry<String, PolicyVersion> childRefPolicy : childStaticRefPolicies.entrySet())
						{
							updateStaticRefPolicies(policyFriendlyId, staticallyReferencedPolicies,
									childRefPolicy.getKey(), childRefPolicy.getValue());
						}
			...
			} else if (policySetChildElt instanceof Policy)
			{
				...
				try
				{
					childPolicy = PolicyEvaluator.getInstance((Policy) policySetChildElt, defaultXPathCompiler,
			 ...
			 * Policy? If there were, the following code would be used.
			 */
		...
		return new PolicySetEvaluator(policyId, policyVersion, policyElement.getTarget(),
				...
				defaultXPathCompiler, expressionFactory, combiningAlgorithmRegistry, longestPolicyRefChain,
				staticallyReferencedPolicies);
	}

	private static void updateStaticRefPolicies(String policyFriendlyId,
			Map<String, PolicyVersion> staticRefPoliciesToUpdate, String newRefPolicyId,
			PolicyVersion newRefPolicyVersion)
	{
