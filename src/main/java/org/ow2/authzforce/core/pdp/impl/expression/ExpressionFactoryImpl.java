...
/**
 * Implementation of ExpressionFactory that supports the Expressions defined in VariableDefinitions in order to resolve
 * VariableReferences. In particular, it makes sure the depth of recursivity of VariableDefinition does not exceed a
 * value (to avoid inconveniences such as stackoverflow or very negative performance impact) defined by
 * {@code maxVarRefDef} parameter to
 * {@link #ExpressionFactoryImpl(DatatypeFactoryRegistry, FunctionRegistry, List, int, boolean, boolean, EnvironmentProperties)}.
 * Note that reference loops are avoided by the fact that a VariableReference can reference only a VariableDefinition
 * defined previously to the VariableReference in this implementation.
 *
 * 
 * @version $Id: $
 */
public final class ExpressionFactoryImpl implements ExpressionFactory
{
	...
	private final Map<String, BaseVariableReference<?>> idToVariableMap = HashObjObjMaps.newMutableMap();
	 ...
	 * <li>A VariableDefinition V1 that uses a VariableReference to VariableDefinition V2 with no further
	 * VariableReference, has a reference depth of 1</li>
	 * <li>etc.</li>
	 * </ul>
	 *
	 ...
	 *            XML/JAXB configurations of Attribute Providers for AttributeDesignator/AttributeSelector evaluation;
	 *            may be null for static expression evaluation (out of context), in which case
	 ...
	 *            represents a VariableReference); strictly negative value means unlimited
	 ...
	 *            the Issuer is not present in the Attribute Designator, but it performs better and is recommended when
	 *            all AttributeDesignators have an Issuer (best practice). Indeed, the XACML 3.0 Attribute Evaluation
	 *            section §5.29 says: "If the Issuer is not present in the AttributeDesignator, then the matching of the
	 *            attribute to the named attribute SHALL be governed by AttributeId and DataType attributes alone."
	 *            Therefore, if {@code strictAttributeIssuerMatch} is false, since policies may use AttributeDesignators
	 *            without Issuer, if the requests are using matching Attributes but with none, one or more different
	 *            Issuers, this PDP engine has to gather all the values from all the attributes with matching
	 *            Category/AttributeId but with any Issuer or no Issuer. Therefore, in order to stay compliant with
	 *            §5.29 and still enforce best practice, when {@code strictAttributeIssuerMatch} is true, we also
	 *            require that all AttributeDesignators set the Issuer field. AttributeDesignators set the Issuer field.
	 * @param environmentProperties
	 *            global PDP configuration environment properties
	 * @throws java.lang.IllegalArgumentException
	 *             If any of attribute Provider modules created from {@code jaxbAttributeProviderConfs} does not provide
	 *             any attribute; or it is in conflict with another one already registered to provide the same or part
	 *             of the same attributes.
	 * @throws java.io.IOException
	 *             error closing the attribute Provider modules created from {@code jaxbAttributeProviderConfs}, when
	 *             and before an {@link IllegalArgumentException} is raised
	 */
	public ExpressionFactoryImpl(final DatatypeFactoryRegistry attributeFactory,
			final FunctionRegistry functionRegistry, final List<AbstractAttributeProvider> jaxbAttributeProviderConfs,
			final int maxVarRefDepth, final boolean allowAttributeSelectors, final boolean strictAttributeIssuerMatch,
	...
	/** {@inheritDoc} */
	@Override
	public VariableReference<?> addVariable(final VariableDefinition varDef, final XPathCompiler xPathCompiler,
			final Deque<String> inoutLongestVarRefChain) throws IllegalArgumentException
	{
		final String varId = varDef.getVariableId();
		/*
		 * Initialize the longest variable reference chain from this VariableDefinition (varDef -> VarDef2 -> ..., where
		 * "v1 -> v2" means: v1's expression contains a VariableReference to v2) as empty for later update by
		 * this#getDefinition() when resolving a VariableReference within this varDef's expression (being parsed just
		 * after). The goal is to detect chains longer than this.maxVariableReferenceDepth to limit abuse of
		 * VariableReferences. There may be multiple VariableReferences in a VariableDefinition's expression, such as an
		 * Apply, and each may be referencing a different VariableDefinition; but we are interested only in the one with
		 * the longest chain of references.
		 */
		/*
		 * we need to check the longest variableReference chain does not have circular reference and does not exceed a
		 * specific value (need to call contains() method repeatedly and preserve the order).
		 */
	...
	/**
	 * Resolves a VariableReference to the corresponding VariableReference(Definition) and validates the depth of
	 * VariableReference, i.e. the length of VariableReference chain. A chain of variable references is a list of
	 * VariableIds, such that V1 -> V2 ->... -> Vn, where 'V1 -> V2' means: V1's Expression contains a VariableReference
	 * to V2.
	 * 
	 * @param jaxbVarRef
	 *            the JAXB/XACML VariableReference with merely identifying a VariableDefinition by its VariableId
	 * 
	 * @return VariableReference containing the resolved VariableDefinition's expression referenced by
	 *         <code>jaxbVarRef</code> as known by this factory, or null if unknown
	 * @param longestVarRefChain
	 *            If we are resolving a VariableReference in a VariableDefinition's expression (may be null if not),
	 *            this is the longest chain of VariableReferences starting from a one in this VariableDefinition. If we
	 *            are not resolving a VariableReference in a VariableDefinition's expression, this may be null.This is
	 *            used to detect exceeding reference depth (see {@link #ExpressionFactoryImpl(int)} for the limit. In a
	 *            Expression such as an Apply, we can have multiple VariableReferences referencing different
	 *            VariableDefinitions. So we can have different depths of VariableReference references. We compare the
	 *            length of the current longest chain with the one we would get by adding the longest one in the
	 *            referenced VariableDefinition and <code>jaxbVarRef</code>'s VariableId. If the latter is longer, its
	 *            content becomes the content <code>longestVarRefChain</code>.
	 * @throws UnknownIdentifierException
	 *             if VariableReference's VariableId is unknown by this factory
	 */
	private BaseVariableReference<?> getVariable(final VariableReferenceType jaxbVarRef,
			final Deque<String> longestVarRefChain) throws IllegalArgumentException
	{
	 ...
	 */
	@Override
	public Function<?> getFunction(final String functionId, final Datatype<?> subFunctionReturnType)
	...
	/** {@inheritDoc} */
	@Override
	public Expression<?> getInstance(final ExpressionType expr, final XPathCompiler xPathCompiler,
			final Deque<String> longestVarRefChain) throws IllegalArgumentException
	{
		final Expression<?> expression;
		/*
		 * We check all types of Expression: <Apply>, <AttributeSelector>, <AttributeValue>, <Function>,
		 * <VariableReference> and <AttributeDesignator>
		 */
		...
		else if (expr instanceof AttributeSelectorType)
		{
			...
			{
				throw new IllegalArgumentException(
						"Unsupported Datatype used in AttributeSelector: " + jaxbAttrSelector.getDataType());
			}
			...
			try
			{
				expression = new AttributeSelectorExpression<>(jaxbAttrSelector, xPathCompiler, attributeProvider,
						attrFactory);
			}
			catch (final XPathExpressionException e)
			{
				throw new IllegalArgumentException(
						"Invalid AttributeSelector's Path='" + jaxbAttrSelector.getPath() + "' into a XPath expression",
						e);
			}
		}
	...
	/** {@inheritDoc} */
	@Override
	public ValueExpression<? extends AttributeValue> getInstance(final AttributeValueType jaxbAttrVal,
