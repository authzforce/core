...
public class PolicyPepActionExpressionsEvaluator {
	...
	private static class ActionExpressionsFactory implements PepActionExpressions.Factory<ActionExpressionsParser> {
		...
		@Override
		public ActionExpressionsParser getInstance(final XPathCompiler xPathCompiler,
	...
	private PolicyPepActionExpressionsEvaluator(final ObligationExpressions jaxbObligationExpressions,
			final AdviceExpressions jaxbAdviceExpressions, final XPathCompiler xPathCompiler,
			final ExpressionFactory expFactory) throws IllegalArgumentException {
		final ActionExpressionsParser actionExpressionsParser = PepActionExpressions.Helper.parseActionExpressions(
				jaxbObligationExpressions, jaxbAdviceExpressions, xPathCompiler, expFactory,
	 ...
	 * ObligationExpressions/AdviceExpressions (a priori specific to a
	 * Policy(Set))
	 *
	 ...
	 *            XPath version
	 * @param expFactory
	 *            Expression factory for parsing the
	 *            AttributeAssignmentExpressions in the Obligation/Advice
	 *            Expressions
	 ...
	 */
	public static PolicyPepActionExpressionsEvaluator getInstance(final ObligationExpressions jaxbObligationExpressions,
			final AdviceExpressions jaxbAdviceExpressions, final XPathCompiler xPathCompiler,
			final ExpressionFactory expFactory) throws IllegalArgumentException {
		...
		return new PolicyPepActionExpressionsEvaluator(jaxbObligationExpressions, jaxbAdviceExpressions, xPathCompiler,
				expFactory);
	}

	/**
	 * Evaluates the PEP action (obligations/Advice) expressions for a given
	 * decision and evaluation context
	 *
	 * @param combiningAlgResult
	 *            Policy(Set) combining algorithm evaluation result; this
	 *            result's decision is used to select the Obligation/Advice
	 *            expressions to apply, i.e. matching on FulfillOn/AppliesTo.
	 *            This result's PEP actions are also merged with the PEP actions
	 *            computed in this method.
	 * @param context
	 *            evaluation context
	 * @return PEP actions (obligations/advices) or null if none
	 * @throws org.ow2.authzforce.core.pdp.api.IndeterminateEvaluationException
	 *             error evaluating one of
	 *             ObligationExpression/AdviceExpressions'
	 *             AttributeAssignmentExpressions' expressions
	 */
	public PepActions evaluate(final DecisionResult combiningAlgResult, final EvaluationContext context)
		...
		return matchingActionExpressions == null ? null
				: PepActionExpressions.Helper.evaluate(matchingActionExpressions, context,
