...
/**
 * XACML Match evaluator. This is the part of the Target that actually evaluates whether the specified attribute values
 * in the Target match the corresponding attribute values in the request context.
 *
 * @version $Id: $
 */
public final class MatchEvaluator
{
			...
			throws IllegalArgumentException
	{
		...
		final Function<BooleanValue> anyOfFunc = (Function<BooleanValue>) expFactory
				.getFunction(StandardFunctions.StdFunctionKey.ANY_OF.getId());
		if (anyOfFunc == null)
		{
			throw new IllegalArgumentException("Unsupported function '"
					+ StandardFunctions.StdFunctionKey.ANY_OF.getId() + "' required for Match evaluation");
		}
