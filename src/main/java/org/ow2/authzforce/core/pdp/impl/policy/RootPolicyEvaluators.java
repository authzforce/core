...
public final class RootPolicyEvaluators {
	...
	private static final class ImmutablePdpDecisionResult implements PdpDecisionResult {
		 ...
		 * core: <i>potential effect value which could have occurred if there
		 * would not have been an error causing the “Indeterminate”</i>. We use
		 ...
		 * <li>{@link DecisionType#NOT_APPLICABLE} is the default value and
		 * means the decision is not Indeterminate, and therefore any extended
		...
		private ImmutablePdpDecisionResult(final DecisionType decision, final DecisionType extendedIndeterminate,
				final Status status, final PepActions pepActions, final EvaluationContext evalCtx) {
		...
		private ImmutablePdpDecisionResult(final DecisionResult decisionResult, final EvaluationContext evalCtx) {
			this(decisionResult.getDecision(), decisionResult.getExtendedIndeterminate(), decisionResult.getStatus(),
					decisionResult.getPepActions(), evalCtx);
		}

		private ImmutablePdpDecisionResult(final Status status, final EvaluationContext evalCtx) {
			this(DecisionType.INDETERMINATE, DecisionType.NOT_APPLICABLE, status, null, evalCtx);
		}
