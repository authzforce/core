...
/**
 * AttributeSelector evaluator, which uses XPath expressions (using Saxon parser) to resolve values from the Request or
 * elsewhere. The AttributeSelector feature in optional in the XACML core specification, and this implementation is
 ...
 * <p>
 * <i>This package provides Saxon's preferred Java API for XSLT, XQuery, XPath, and XML Schema processing. The interface
 * is designed to hide as much as possible of the detail of the implementation. However, the API architecture faithfully
 * reflects the internal architecture of the Saxon product, unlike standard APIs such as JAXP and XQJ which in many
 * cases force compromises in the design and performance of the application.</i>
 * </p>
 * </li>
 *
 * <li>Functional: s9api provides XPATH 3.0 support, whereas standard Java APIs designed for XPATH 1.0 support only. See
 * http://www.saxonica.com/html/documentation/conformance/jaxp.html. However, for the moment, only XPath 1.0 and 2.0 are
 * supported by this class. But we prepare for the future.</li>
 * </ol>
 * </p>
 *
 * @param <AV>
...
public final class AttributeSelectorExpression<AV extends AttributeValue> implements Expression<Bag<AV>>
{
	...
	private static final class AttributeDefinedContextNodeSelector implements ContextNodeSelector
	{
		...
		private AttributeDefinedContextNodeSelector(final String contentNodeSrcAttributeCategory,
				final AttributeGUID contextSelectorGUID, final AttributeProvider attrProvider,
				final XPathCompiler xPathCompiler, final String missingContextSelectorAttributeExceptionMessage,
				final IndeterminateEvaluationException missingAttributeForUnknownReasonException)
		{
			...
			this.xpathEvalErrMsgSuffix = "' from ContextSelectorId='" + contextSelectorId
					+ "' against Content of Attributes of Category=" + contentNodeSrcAttributeCategory;
		...
		{
			final Bag<XPathValue> bag = attrProvider.get(contextSelectorId,
					StandardDatatypes.XPATH_FACTORY.getDatatype(), context);
			if (bag == null)
			{
			...
			if (bag.isEmpty())
			{
				throw new IndeterminateEvaluationException(missingContextSelectorAttributeExceptionMessage,
			...
			catch (final SaxonApiException e)
			{
				throw new IndeterminateEvaluationException(
						this + ": Error evaluating XPath='" + contextSelectorPath + xpathEvalErrMsgSuffix,
			...
			if (contextNode == null)
			{
				throw new IndeterminateEvaluationException(this + ": No node returned by evaluation of XPath='"
						+ contextSelectorPath + xpathEvalErrMsgSuffix, StatusHelper.STATUS_SYNTAX_ERROR);
			}
	...
	private static AttributeValueType xdmToJaxbAttributeValue(final String attrDatatype, final XdmNode node)
			throws IllegalArgumentException
	{
		...
		switch (node.getNodeKind()) {
			case ATTRIBUTE:
				/*
				 * We only take the attribute value. (For XPath getting an attribute Value, the result XdmNode still
				 * holds the attribute QName.)
				 */
			case TEXT:
			...
			/*
			 * TODO: the commented cases below are more complex to handle. Further checking/testing is required before
			 * providing support for them. But first of all, are these cases worth the trouble? Find a few good use
			 * cases for them. In the meantime, do not remove these lines of code below, unless to rewrite/refactor with
			 * same quality level.
			 */
			...
			default:
				throw new IllegalArgumentException(
						"Cannot create AttributeValue from XML node (type not supported): " + getDescription(node));
		}
	 ...
	 * @param xPathCompiler
	 *            XPATH compiler used for compiling {@code attrSelectorElement.getPath()} and XPath given by
	 *            {@code attrSelectorElement.getContextSelectorId()} if not null
	 * @param attrProvider
	 *            AttributeProvider for finding value of the attribute identified by ContextSelectorId in
	 *            {@code attrSelectorElement}; may be null if ContextSelectorId not specified
	 * @param attrFactory
	 *            attribute factory to create the AttributeValue(s) from the XML node(s) resolved by XPath
	 * @throws javax.xml.xpath.XPathExpressionException
	 *             if the Path could not be compiled to an XPath expression (using <code>namespaceContextNode</code> if
	 *             non-null)
	 * @throws java.lang.IllegalArgumentException
	 *             if {@code attrSelectorElement}, {@code xpathCompiler} or {@code attrFactory} is null; or
	 *             ContextSelectorId is not null but {@code attrProvider} is null
	 */
	public AttributeSelectorExpression(final AttributeSelectorType attrSelectorElement,
			final XPathCompiler xPathCompiler, final AttributeProvider attrProvider,
			final DatatypeFactory<AV> attrFactory) throws XPathExpressionException, IllegalArgumentException
	{
		...
		if (attributeSelectorId.getContextSelectorId() == null)
		{
			...
			this.xpathEvalExceptionMessage = this
					+ ": Error evaluating XPath against XML node from Content of Attributes Category='"
					+ attributeCategory + "'";
		...
		else
		{
			...
			final AttributeGUID contextSelectorGUID = new AttributeGUID(attributeSelectorId.getCategory(), null,
					contextSelectorId);
			final String missingContextSelectorAttributeExceptionMessage = this
					+ ": No value found for attribute designated by Category=" + attributeCategory
					+ " and ContextSelectorId=" + contextSelectorId;
			final IndeterminateEvaluationException missingAttributeForUnknownReasonException = new IndeterminateEvaluationException(
					missingAttributeMessage + " for unknown reason", StatusHelper.STATUS_MISSING_ATTRIBUTE);
			this.contextNodeSelector = new AttributeDefinedContextNodeSelector(attributeCategory, contextSelectorGUID,
					attrProvider, xPathCompiler, missingContextSelectorAttributeExceptionMessage,
					missingAttributeForUnknownReasonException);
			this.xpathEvalExceptionMessage = this
					+ ": Error evaluating XPath against XML node from Content of Attributes Category='"
					+ attributeCategory + "' selected by ContextSelectorId='" + contextSelectorId + "'";
			this.xpathCompiler = xPathCompiler;
		}
		...
		this.missingAttributeBecauseNullContextException = new IndeterminateEvaluationException(
				"Missing Attributes/Attribute for evaluation of AttributeDesignator '" + this.attributeSelectorId
		...
		this.missingAttributesContentException = new IndeterminateEvaluationException(
				this + ": No <Content> element found in Attributes of Category=" + attributeCategory,
				StatusHelper.STATUS_SYNTAX_ERROR);

		this.mustBePresentEnforcer = attrSelectorElement.isMustBePresent()
				? new Bags.NonEmptinessValidator(missingAttributeMessage) : Bags.DUMB_VALIDATOR;
	}
	...
	public Bag<AV> evaluate(final EvaluationContext context) throws IndeterminateEvaluationException
	{
		...
		try
		{
			...
			/*
			 * An XPathExecutable is immutable, and therefore thread-safe. It is simpler to load a new XPathSelector
			 * each time the expression is to be evaluated. However, the XPathSelector is serially reusable within a
			 * single thread. See Saxon Javadoc.
			 */
			...
			{
				throw new IndeterminateEvaluationException(this.xpathEvalExceptionMessage,
						StatusHelper.STATUS_SYNTAX_ERROR, e);
			}
			...
			for (final XdmItem xpathEvalResultItem : xpathEvalResult)
			{
				...
				if (xpathEvalResultItem instanceof XdmAtomicValue)
				{
					...
					jaxbAttrVal = new AttributeValueType(Collections.<Serializable>singletonList(strVal),
							attributeDatatype.getId(), null);
				}
				...
				else
				{
					final String contextSelectorId = attributeSelectorId.getContextSelectorId();
					throw new IndeterminateEvaluationException(
							this + ": Invalid type of result #" + xpathEvalResultItemIndex
									+ " from evaluating XPath against XML node from Content of Attributes Category='"
									+ attributeSelectorId.getCategory()
									+ (contextSelectorId == null ? ""
											: "' selected by ContextSelectorId='" + contextSelectorId + "'")
									+ xpathEvalResultItem.getClass().getName(),
							StatusHelper.STATUS_SYNTAX_ERROR);
				}
	...
	{
		/*
		 * Because this class is immutable (excluding toString which is derived from others), we can do caching (and
		 * lazy init) of this method result.
		 */
		if (toString == null)
		{
			toString = "AttributeSelector [" + this.attributeSelectorId + ", dataType= "
					+ this.returnType.getElementType() + ", mustBePresent= "
