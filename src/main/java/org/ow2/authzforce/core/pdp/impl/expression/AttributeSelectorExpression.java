...
public class AttributeSelectorExpression<AV extends AttributeValue> extends AttributeSelectorType implements Expression<Bag<AV>>
{
	...
	private static AttributeValueType xdmToJaxbAttributeValue(String attrDatatype, XdmNode node) throws IllegalArgumentException
	{
		...
		/*
		 * TODO: the commented cases below are more complex to handle. Further checking/testing is required before providing support for them. But first of all, are these cases worth the trouble? Find
		 * a few good use cases for them. In the meantime, do not remove these lines of code below, unless to rewrite/refactor with same quality level.
		 */
			...
			throws XPathExpressionException, IllegalArgumentException
	{
		...
		} else
		{
			...
			this.xpathEvalExceptionMessage = this + ": Error evaluating XPath against XML node from Content of Attributes Category='" + category + "' selected by ContextSelectorId='"
					+ contextSelectorId + "'";
		...
		this.missingAttributeBecauseNullContextException = new IndeterminateEvaluationException("Missing Attributes/Attribute for evaluation of AttributeDesignator '" + this.id
	...
	public Bag<AV> evaluate(EvaluationContext context) throws IndeterminateEvaluationException
	{
		...
		try
		{
			...
			} else
			{
				...
				{
					throw new IndeterminateEvaluationException(this + ": Error evaluating XPath='" + contextSelectorPath + "' from ContextSelectorId='" + contextSelectorId
							+ "' against Content of Attributes of Category=" + category, StatusHelper.STATUS_SYNTAX_ERROR, e);
				}

				if (contextNode == null)
				{
					throw new IndeterminateEvaluationException(this + ": No node returned by evaluation of XPath='" + contextSelectorPath + "' from ContextSelectorId='" + contextSelectorId
							+ "' against Content of Attributes of Category=" + category, StatusHelper.STATUS_SYNTAX_ERROR);
				}
			}
			...
			for (final XdmItem xpathEvalResultItem : xpathEvalResult)
			{
				...
				} else if (xpathEvalResultItem instanceof XdmNode)
				{
					...
					{
						throw new IndeterminateEvaluationException(this + ": Error creating attribute value of type '" + dataType + "' from result #" + xpathEvalResultItemIndex
	...
	public String toString()
	{
		...
		if (toString == null)
		{
			toString = "AttributeSelector [" + (category != null ? "category=" + category + ", " : "") + (contextSelectorId != null ? "contextSelectorId=" + contextSelectorId + ", " : "")
					+ (path != null ? "path=" + path + ", " : "") + (dataType != null ? "dataType=" + dataType + ", " : "") + "mustBePresent=" + mustBePresent + "]";
		}
