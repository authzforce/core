...
public abstract class IndividualDecisionRequestEvaluator
{
	...
	{
		/**
		 * Return an updatable map after merging {@code pdpIssuedAttributes} and {@code requestAttributes} or one of
		 * each into it, depending on the implementation
		 * 
	...
	private static final Map<AttributeGUID, Bag<?>> STD_ENV_RESET_MAP = HashObjObjMaps
			.<AttributeGUID, Bag<?>>newImmutableMapOf(StandardEnvironmentAttributes.CURRENT_DATETIME_ATTRIBUTE_GUID,
					Bags.empty(StandardDatatypes.DATETIME_FACTORY.getDatatype(),
							newReqMissingStdEnvAttrException(
									StandardEnvironmentAttributes.CURRENT_DATETIME_ATTRIBUTE_GUID)),
					StandardEnvironmentAttributes.CURRENT_DATE_ATTRIBUTE_GUID,
					Bags.empty(StandardDatatypes.DATE_FACTORY.getDatatype(),
							newReqMissingStdEnvAttrException(
									StandardEnvironmentAttributes.CURRENT_DATE_ATTRIBUTE_GUID)),
					StandardEnvironmentAttributes.CURRENT_TIME_ATTRIBUTE_GUID,
					Bags.empty(StandardDatatypes.TIME_FACTORY.getDatatype(), newReqMissingStdEnvAttrException(
							StandardEnvironmentAttributes.CURRENT_TIME_ATTRIBUTE_GUID)));
	...
	private static final RequestAndPdpIssuedNamedAttributesMerger REQUEST_OVERRIDES_ATTRIBUTES_MERGER = new RequestAndPdpIssuedNamedAttributesMerger()
	{
		...
		{
			/*
			 * Request attribute values override PDP issued ones. Do not modify pdpIssuedAttributes directly as this may
			 * be used for other requests (Multiple Decision Profile) as well. so we must not modify it but clone it
			 * before individual decision request processing.
			 */
			...
			/**
			 * 
			 * XACML standard (§10.2.5) says: "If values for these [the standard environment attributes, i.e.
			 * current-time, current-date, current-dateTime] attributes are not present in the decision request, then
			 * their values MUST be supplied by the context handler". In our case, "context handler" means the PDP. In
			 * other words, the attribute values come from request by default, or from the PDP if (and *only if* in this
			 * case) they are not set in the request. More precisely, if any of these standard environment attributes is
			 * provided in the request, none of the PDP values is used, even if some policy requires one that is missing
			 * from the request. Indeed, this is to avoid such case when the decision request specifies at least one
			 * date/time attribute, e.g. current-time, but not all of them, e.g. not current-dateTime, and the policy
			 * requires both the one(s) provided and the one(s) not provided. In this case, if the PDP provides its own
			 * value(s) for the missing attributes (e.g. current-dateTime), this may cause some inconsistencies since we
			 * end up having date/time attributes coming from two different sources/environments (current-time and
			 * current-dateTime for instance).
			 */
			if (requestAttributes.containsKey(StandardEnvironmentAttributes.CURRENT_DATETIME_ATTRIBUTE_GUID)
					|| requestAttributes.containsKey(StandardEnvironmentAttributes.CURRENT_DATE_ATTRIBUTE_GUID)
					|| requestAttributes.containsKey(StandardEnvironmentAttributes.CURRENT_TIME_ATTRIBUTE_GUID))
			{
				/*
				 * Request has at least one standard env attribute -> make sure all PDP values are ignored (overridden
				 * by STD_ENV_RESET_MAP no matter whether requestAttributes contains all of them or not)
				 */
	...
	private static final RequestAndPdpIssuedNamedAttributesMerger PDP_OVERRIDES_ATTRIBUTES_MERGER = new RequestAndPdpIssuedNamedAttributesMerger()
	{
				...
				final Map<AttributeGUID, Bag<?>> requestAttributes)
		{
			...
			/*
			 * Do not modify pdpIssuedAttributes directly as this may be used for other requests (Multiple Decision
			 * Profile) as well. so we must not modify it but clone it before individual decision request processing.
			 */
			if (pdpIssuedAttributes == null)
			{
				return requestAttributes == null ? null : HashObjObjMaps.newUpdatableMap(requestAttributes);
	 ...
	 *            (mandatory) Defines the source for the standard environment attributes specified in §10.2.5:
	 *            current-time, current-date and current-dateTime. The options are:
	 *            <ul>
	 *            <li>REQUEST_ELSE_PDP: the default choice, that complies with the XACML standard (§10.2.5): "If values
	 *            for these attributes are not present in the decision request, then their values MUST be supplied by
	 *            the context handler", in our case, "context handler" means the PDP. In other words, the attribute
	 *            values come from request by default, or from the PDP if (and *only if* in this case) they are not set
	 *            in the request. Issue: what if the decision request only specifies current-time but not
	 *            current-dateTime, and the policy requires both? Should the PDP provides its own value for
	 *            current-dateTime? This could cause some inconsistencies since current-time and current-dateTime would
	 *            come from two different sources/environments. With this option, we have a strict interpretation of the
	 *            spec, i.e. if any of these attribute is not set in the request, the PDP uses its own value instead. So
	 *            BEWARE. Else you have the other options below.</li>
	 *            <li>REQUEST_ONLY: always use the value from the request, or nothing if the value is not set in the
	 *            request, in which case this results in Indeterminate (missing attribute) if the policy evaluation
	 *            requires it.</li>
	 *            <li>PDP_ONLY: always use the values from the PDP. In other words, Request values are simply ignored;
	 *            PDP values systematically override the ones from the request. This also guarantees that they are
	 *            always set (by the PDP). NB: note that the XACML standard (§10.2.5) says: "If values for these
	 *            attributes are not present in the decision request, then their values MUST be supplied by the context
	 *            handler" but it does NOT say "If AND ONLY IF values..." So this option could still be considered XACML
	 *            compliant in a strict sense.</li>
	 *            </ul>
	 ...
	 * @param pdpIssuedAttributes
	 *            a {@link java.util.Map} of PDP-issued attributes including at least the standard environment
	 *            attributes: current-time, current-date, current-dateTime.
