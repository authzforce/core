...
public class BasicV3_2
{
	 ...
	 */
	private static final Logger LOGGER = LoggerFactory.getLogger(BasicV3_2.class);
	/**
	 * The map of results
	 */
	...
	public void testBasicTest0002() throws Exception
	{
		...
		String reqResNo;
		final String policyFilename = "TestPolicy_0002.xml";
		...
		request = TestUtils.createRequest(ROOT_DIRECTORY, VERSION_DIRECTORY, "request_0002_" + reqResNo + ".xml");
		if (request != null)
		{
			...
			if (response != null)
			{
				...
				expectedResponse = TestUtils.createResponse(ROOT_DIRECTORY, VERSION_DIRECTORY, "response_0002_" + reqResNo + ".xml");
				if (expectedResponse != null)
				{
					boolean assertion = TestUtils.match(response, expectedResponse);
					if (assertion)
					{
						LOGGER.debug("Assertion SUCCESS for: TestPolicy_0002.xml and response_0002_" + reqResNo);
						results.put("response_0002_" + reqResNo, "SUCCESS");
					} else
					{
						LOGGER.error("Assertion FAILED for: TestPolicy_0002 and response_0002_" + reqResNo);
