...
public class BasicV3_4
{
	 ...
	 */
	private static final Logger LOGGER = LoggerFactory.getLogger(BasicV3_4.class);
	/**
	 * The map of results
	 */
	...
	public void testBasicTest0004() throws Exception
	{
		...
		String reqResNo;
		String policyFilename = "TestPolicy_0004.xml";
		...
		request = TestUtils.createRequest(ROOT_DIRECTORY, VERSION_DIRECTORY, "request_0004_" + reqResNo + ".xml");
		if (request != null)
		{
			...
			if (response != null)
			{
				...
				expectedResponse = TestUtils.createResponse(ROOT_DIRECTORY, VERSION_DIRECTORY, "response_0004_" + reqResNo + ".xml");
				if (expectedResponse != null)
				{
					boolean assertion = TestUtils.match(response, expectedResponse);
					if (assertion)
					{
						LOGGER.debug("Assertion SUCCESS for: IIIA" + "response_0004_" + reqResNo);
						results.put("response_0004_" + reqResNo, "SUCCESS");
					} else
					{
						LOGGER.error("Assertion FAILED for: TestPolicy_0004 and response_0004_" + reqResNo);
