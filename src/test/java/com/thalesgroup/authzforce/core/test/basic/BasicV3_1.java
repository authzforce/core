...
public class BasicV3_1
{
	 ...
	 */
	private static final Logger LOGGER = LoggerFactory.getLogger(BasicV3_1.class);
	/**
	 * The map of results
	 */
	...
	public void testBasicTest0001() throws Exception
	{
		...
		String reqResNo;
		String policyFilename = "TestPolicy_0001.xml";
		...
		request = TestUtils.createRequest(ROOT_DIRECTORY, VERSION_DIRECTORY, "request_0001_" + reqResNo + ".xml");
		if (request != null)
		{
			...
			if (response != null)
			{
				...
				expectedResponse = TestUtils.createResponse(ROOT_DIRECTORY, VERSION_DIRECTORY, "response_0001_" + reqResNo + ".xml");
				if (expectedResponse != null)
				{
					boolean assertion = TestUtils.match(response, expectedResponse);
					if (assertion)
					{
						LOGGER.debug("Assertion SUCCESS for: IIIA" + "response_0001_" + reqResNo);
						results.put("response_0001_" + reqResNo, "SUCCESS");
					} else
					{
						LOGGER.error("Assertion FAILED for: TestPolicy_0001.xml and response_0001_" + reqResNo);
