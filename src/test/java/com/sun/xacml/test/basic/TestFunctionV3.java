...
/**
 * This XACML 3.0 basic XACML function. This would test new functions that is
 * introduced with XACML 3.0.
 */
public class TestFunctionV3 extends TestCase {
		...
		for (int i = 1; i < 4; i++) {
			...
			if (request != null) {
				...
				ResponseCtx response = TestUtil.evaluate(
						getPDPNewInstance(policies), request, Integer.parseInt(XACMLAttributeId.XACML_VERSION_3_0.value()));
				if (response != null) {
	...
	private static PDP getPDPNewInstance(Set<String> policies) {
		...
		DirectoryPolicyFinder testPolicyFinderModule = new DirectoryPolicyFinder(
				policyLocations);
		...
		// PDPConfig pdpConfig = new PDPConfig(pdpConfig.getAttributeFinder(),
		// finder, pdpConfig.getResourceFinder(), false);
		return pdp;
	...
	private static PDP loadPdp() throws FileNotFoundException {
		...
		PDP myPdp = null;
		try {
			properties.load(AdvanceTestV3.class.getClassLoader()
					.getResourceAsStream("authzforce.test.properties"));
		} catch (IOException e) {
